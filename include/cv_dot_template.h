...
#define CV_CANDIDATE_STRUCT
	...
	cv_candidate * cv_dot_template<M, N, S, G>::online_comp(Ipp8u * ap_pix, int a_thres)
	{
		...
		while (lp_cur != NULL)
		{
			...
			if (l_cur_val >= a_thres && l_cur_val > l_max) // find the candidate with the max matching value
			{
				bool l_flag = false;
				...
				lp_candidate->m_clu = lp_cur->m_clu;
				lp_candidate->m_cla = lp_cur->m_cla;
				lp_candidate->m_val = l_max = l_cur_val; // update currently known maximum matching value
			}
			lp_cur = lp_cur->mp_nxt;
		}
		return lp_candidate;
		...
		int a_height)
	{
		...
		for (int l_r = 0; l_r < a_height - M; ++l_r)
		{
			for (int l_c = 0; l_c < a_width - N; ++l_c)
			{
				cv_candidate * lp_candidate = this->online_comp(lp_pix_col, a_thres); // find the candidate template with max matching score
	...
	std::pair<Ipp8u*, Ipp32f*> cv_dot_template<M, N, S, G>::compute_gradients(IplImage * ap_img, int a_num_of_gradients)
	{
		...
		ippsCopy_32f((Ipp32f*)lp_sobel_mg->imageData, lp_peak_mag, lp_sobel_mg->width*lp_sobel_mg->height); // copy operation: (src, dest, length)
